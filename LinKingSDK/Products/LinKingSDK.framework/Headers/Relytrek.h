...
@protocol RelytrekDelegate <NSObject>
@optional

/// 创建订单成功回调

- (void)Ingredientgradient:(NSString * _Nullable)orderId withError:(NSError *_Nullable)error;
...
@interface Relytrek : NSObject
@property (nonatomic, weak) id <RelytrekDelegate> delegate;

+ (instancetype)shared;
/// 拉取所有商品信息
- (void)Derailmethane:(void(^_Nullable)(NSError * _Nullable error, NSArray*_Nullable products,NSArray * _Nullable invalidProducts))complete;
//开始内购
- (void)Drawbackravine:(NSString *)purchID parames:(NSDictionary *)parames completeHandle:(IAPCompletionHandle)handle;
// 查询订阅
